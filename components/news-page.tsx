...
import { toast } from "@/hooks/use-toast" // Assuming useToast is available for notifications
        ...
        {newsItems.map((item, index) => (
          <div key={item.id} className="flex relative">
            ...
            <div className="flex flex-col items-center mr-4">
              ...
              {index < newsItems.length - 1 && <div className="w-0.5 bg-blue-200 flex-grow mt-2 mb-2"></div>}
            </div>

            {/* News Card */}
            <Card className="glass-card border-white/50 flex-1 data-card">
              <CardHeader className="pb-3">
                <CardTitle className="text-slate-800 text-lg font-semibold">{item.title}</CardTitle>
              ...
              <CardContent className="pt-0">
                <p className="text-slate-700 text-sm leading-relaxed mb-4">{item.content}</p>
