...
import { GraduationCap, Play, Lock, ExternalLink, Star, TrendingUp, Shield, Target } from "lucide-react"
      ...
      <div className="px-6 -mt-4 space-y-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-4 mb-4">
              <Avatar className="w-16 h-16">
                ...
                <AvatarFallback className="glass-card-strong text-slate-700 font-bold">DT</AvatarFallback>
              </Avatar>
              <div className="flex-1">
                <h3 className="text-slate-800 font-bold text-lg">Donato狐总</h3>
                <p className="text-slate-600 text-sm mb-2">WEB3职业交易员 · 数十年量化交易经验</p>
            ...
            {features.map((feature, index) => {
              ...
              return (
                <div key={index} className="flex items-start space-x-3 p-3 glass-card rounded-lg">
                  ...
                  <div>
                    <h4 className="text-slate-800 font-medium mb-1">{feature.title}</h4>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      <Badge className="text-xs bg-blue-100/80 text-blue-700 border-blue-200">{course.level}</Badge>
                    </div>
                    <h3 className="text-slate-800 font-semibold text-lg mb-2">{course.title}</h3>
                    <p className="text-slate-600 text-sm mb-3 leading-relaxed">{course.description}</p>
        ...
        <Card className="glass-card border-white/30">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center space-x-3">
              ...
              <p className="text-slate-700 text-sm">具备技术分析基础，想落地交易系统的操盘者</p>
            </div>
            <div className="flex items-center space-x-3">
              <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
              <p className="text-slate-700 text-sm">有策略框架但执行效率低、选币耗时的交易员</p>
            </div>
            <div className="flex items-center space-x-3">
              <div className="w-2 h-2 bg-green-500 rounded-full"></div>
              <p className="text-slate-700 text-sm">具备趋势交易思维，想提升胜率与纪律性的进阶用户</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="glass-card border-white/50">
          <CardHeader>
            ...
            <p className="text-slate-600 text-sm ml-11">基于机构行为的趋势系统+程序化工具，清晰捕捉中期趋势</p>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {signalExamples.map((example, index) => (
                <div key={index} className="glass-card rounded-xl overflow-hidden border-white/50">
                  <Image
                  ...
                  />
                  <div className="p-4">
                    <h5 className="text-slate-800 font-semibold text-base mb-2">{example.title}</h5>
                    <Badge className="bg-red-100/80 text-red-700 border-red-300">需要学员权限</Badge>
                  </div>
                </div>
              ))}
            </div>
        ...
        <Card className="glass-card-strong border-white/50">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-slate-600 text-sm mb-4">程序化辅助工具：学员专属【筛选系统】，落地模型应用</p>
            <Button className="diffused-button text-white font-semibold border-0">获得机构级策略</Button>
          </CardContent>
        </Card>
      </div>
    </div>
